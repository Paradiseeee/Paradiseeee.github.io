...
![输出结果](/img/in-post/presentation.jpg)

![](./img/in-post/presentation.jpg)

## 总结

从结果可以看到卷积运算成功的描绘出了图像中存在的边缘变化，并且由于卷积核为水平检测，得到的输出图像中偏向水平方向的边缘比垂直方向的边缘更加明显。

本例由于没有对迭代过程进行向量化操作，运行时间偏长。对于1920*1080分辨率的图片，处理时间大概为一分钟，有很大的优化空间。在python的scipy和numpy中集成了更加高效的函数可以直接处理，本例只作为理解卷积原理的练习。
