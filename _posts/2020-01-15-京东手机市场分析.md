...
> https://list.jd.com/list.html?cat=9987,653,655&page=1&stock=0&sort=sort_rank_asc&trans=1&JL=4_7_0#J_main
...
> https://item.jd.com/{Product_ID}.html#product-detail

![商品详情页](/img/in-post/DA4B/jd_page.jpg)
...
def get_info(driver):
    ...
    for i in range(1,61):
        # 获取 outerHTML
        html = driver.find_element_by_xpath(xpath.format(i)).get_attribute('outerHTML')
        # 解析相应字段内容
        id = re.compile(r'data-sku="(.*?)"').findall(id_html)[0]
        price = driver.find_element_by_xpath('//*[@id="plist"]/ul/li[{}]/div/div[3]'.format(i)).text[1:]
        # ... 
        # 解析以及保存信息
...
if __name__ == '__main__':
    ...
    URL = 'https://list.jd.com/list.html?cat=9987,653,655&page={}&stock=0&sort=sort_rank_asc&trans=1&JL=4_7_0#J_main'
...
def get_params(html):
    ...
    # 增加品牌字段
    brand = BeautifulSoup(html.replace('\n', ''),
                        'html.parser').find_all('ul',
                        attrs={'id': 'parameter-brand'})[0].text.replace(' ', '').split('：')
    params[brand[0]] = brand[1]
    # 包含所有参数的文本
    regex = r'<div class="Ptable-item">(.*?)<div class="package-list">'
    dls = BeautifulSoup(re.compile(regex).findall(html.replace('\n' ,'|'))[0], 
                        'html.parser').find_all('dl', 
                        attrs={'class': 'clearfix'})
    params['all_params_raw_text'] = '||'.join([dl.text.replace(' ', '') for dl in dls])
...
name_list = ['品牌', '产品名称', '机身颜色', 'CPU型号', 
            '屏幕尺寸(inch)', '后置主摄像素(万)', '前置主摄像素(万)', '电池容量(mAh)']
...
```python
# 由于对于不同的模型算法，对数据有不同清洗需求，不能一概而论。使用以下函数进行初步的数据清洗，保留大部分信息。
...
def parse_numeric(name, leng, ulimit, llimit, df):
    ...
    df[name] = [t[:leng] if len(re.compile(r'\.').findall(t))>1 else t for t in df[name]]
...
```
```python
engine = sqlalchemy.create_engine(f'mysql+pymysql://{usr}:{pw}@localhost:3306/{db}?charset=utf8')
...
![数据预览](/img/in-post/DA4B/data_views.jpg)

### （2）详细统计报告
&emsp;&emsp;&emsp;&emsp;[<font size=5>查看详细报告</font>](/img/in-post/DA4B/profile.html)