...
web <- read_html(url, encoding='utf-8')		# 与网页对应的编码，可以在<head>中查看到
View(web)
# 使用 View 将在 RStudio 的编辑器中打开 HTML 文档的树状图
# 这是点击对应元素节点最右边的一个小图标，可以将对应的解析代码发送到 Console
# 这一点非常方便好用，因为 R 自动将 HTML 文档解析为可以操作的数据类型
# 而不需要手动去解析
# 这里有点像 Python 的 BeautifulSoup，但是 R 是可以直接在图形界面傻瓜式操作的
```
...
# 创建 connection
create_conn <- function(password, dbname){
	conn <- dbConnect(MySQL(), host='localhost', user='root', 
