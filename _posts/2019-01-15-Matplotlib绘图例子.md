...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_8_0.png)
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_11_0.png)
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_15_0.png)
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_16_0.png)
...
#导入数据>>https://raw.githubusercontent.com/selva86/datasets/master/mpg_ggplot2.csv
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_19_0.png)
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_22_0.png)
...
#main axe上画散点图
ax_main.scatter('displ', 'hwy', c=df.manufacturer.astype('category').cat.codes,
               s=df.cty*4, alpha=0.9, data=df, cmap='tab10',edgecolors='gray',
               linewidths=0.5)
#右方的直方图
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_24_0.png)
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_26_0.png)
...
```python
'''相关图用于直观地查看给定数据框（二维数组）中所有的变量对之间的两两相关度量'''
#导入数据
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_29_0.png)
...
#数据准备
x = df.loc[:, ['mpg']]#抽取一列
df['mpg_z'] = (x - x.mean())/x.std()#计算 每个数据点 相对于标准差 的离散程度
df['colors'] = ['red' if i<0 else 'green' for i in df['mpg_z']]#分配颜色，增加颜色变量
df.sort_values('mpg_z', inplace=True)#按计算所得的 离散程度 重排数据
df.reset_index(inplace=True)#重排后 更新index
print(df.head())
```
...
plt.hlines(y=df.index, xmin=0, xmax=df.mpg_z, color=df.colors, alpha=0.5, linewidth=5)
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_38_0.png)
...
for x, y, tex in zip(df.mpg_z, df.index, df.mpg_z):
    t = plt.text(x, y, round(tex, 2), horizontalalignment='right' if x<0 else \
                 'left', verticalalignment='center', fontdict={'color': 'red'\
                if x<0 else 'green', 'size': 14}
                )
#修饰图片
plt.yticks(df.index, df.cars, fontsize=12)
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_40_0.png)
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_42_0.png)
...
plt.annotate('Mercedes Models', xy=(0.0, 11.0), xytext=(1.0, 11), xycoords='data',
            ...
            arrowprops=dict(arrowstyle='-[, widthB=2.0, lengthB=1.5', lw=2.0,
                           color='steelblue'),
            )

#添加补丁（框框）
from matplotlib import patches
p1 = patches.Rectangle((-0.2, -1), width=0.3, height=3, alpha=0.2, facecolor='red')
p2 = patches.Rectangle((1.5, 27), width=.8, height=5, alpha=0.2, facecolor='green')
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_45_0.png)
...
#修饰图片
xtickvals = [str(m)[:3].upper()+'-'+str(y) for y,m in zip(df.date.dt.year,
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_48_0.png)
...
#抽取两列，进行排序，并按照排序求均值
df = df_raw[['cty', 'manufacturer']].groupby('manufacturer').apply(lambda x:
                                                                  x.mean())
...
plt.xticks(df.index, df.manufacturer.str.upper(), rotation=60, fontsize=12, 
          horizontalalignment='right')
#添加补丁
p1 = patches.Rectangle((0.57, -0.005), width=0.33, height=0.13, alpha=0.1, 
                      facecolor='green', transform=fig.transFigure)
p2 = patches.Rectangle((0.124, -0.005), width=0.446, height=0.13, alpha=0.1,
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_52_0.png)
...
for row in df.itertuples():
    ax.text(row.Index, row.cty+0.5, s=round(row.cty, 2), horizontalalignment=
           'center', verticalalignment='bottom', fontsize=14)
plt.show()
```


![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_54_0.png)
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_56_0.png)
...
def newline(p1, p2, color='black'):
    ax = plt.gca()
    line = lines.Line2D([p1[0],p2[0]], [p1[1],p2[1]], marker='o', markersize=6,
...
#画垂线
ax.vlines(x=1, ymin=500, ymax=13000, color='black', alpha=0.7, linewidth=1, 
         linestyles='dotted')
ax.vlines(x=3, ymin=500, ymax=13000, color='black', alpha=0.7, linewidth=1,
         linestyles='dotted')
#散点
...
for p1,p2,c in zip(df['1952'], df['1957'], df['continent']):
    newline([1,p1], [3,p2])
    ax.text(1-0.05, p1, c+','+str(round(p1)), fontsize=14,
           horizontalalignment='right', verticalalignment='center')
    ax.text(3+0.05, p2, c+','+str(round(p2)), fontsize=14,
           horizontalalignment='left', verticalalignment='center')
#前后的注释
ax.text(1-0.05, 13000, 'BEFORE', fontdict={'size': 18, 'weight': 700}, 
        horizontalalignment='right', verticalalignment='center')
ax.text(3+0.05, 13000, 'AFTER', fontdict={'size': 18, 'weight': 700},
        horizontalalignment='left', verticalalignment='center')
#修饰图片
ax.set_title("SlopeChart: Compairing GDP Per Capita between 1952 and 1957",
            fontsize=22)
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_60_0.png)
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_64_0.png)
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_68_0.png)
...
plt.figure(figsize=(16,9), dpi=80)
colors = [plt.cm.Spectral(i/float(len(vals)-1)) for i in range(len(vals))]
n, bins, patches = plt.hist(vals, df[x_var].unique().__len__(), stacked=True, 
...
plt.ylabel('Frequency')
plt.ylim(0, 40)
plt.xticks(ticks=bins, labels=np.unique(df[x_var]).tolist(), rotation=90, 
           horizontalalignment='left')
plt.show()
```


![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_71_0.png)
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_73_0.png)
...
sns.distplot(df.loc[df['class']=='compact', 'cty'], color='dodgerblue', label=
            'compact', hist_kws={'alpha': 0.7}, kde_kws={'linewidth': 3})
sns.distplot(df.loc[df['class']=='suv', 'cty'], color='orange', label=
            'SUV', hist_kws={'alpha': 0.7}, kde_kws={'linewidth': 3})
sns.distplot(df.loc[df['class']=='minivan', 'cty'], color='g', label="minivan",
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_75_0.png)
...
```python
'''Joy Plot 允许不同组的密度曲线重叠，这是一种可视化大量分组数据的彼此关系的好
方法。并且看起来更加悦目，更清晰地传达正确的信息'''
import joypy
#数据准备
...
df.reset_index(inplace=True)
df_median = df_raw[['cty', 'manufacturer']].groupby('manufacturer').apply(
    lambda x: x.median())
```
...
#绘图
fig, ax = plt.subplots(figsize=(16,10), dpi=80)
ax.hlines(y=df.index, xmin=0, xmax=40, color='gray', alpha=0.5, linewidth=0.5, 
          linestyle='dashdot')
#画包点
for i,make in enumerate(df.manufacturer):
    df_make = df_raw.loc[df_raw.manufacturer==make,:]
    ax.scatter(y=np.repeat(i,df_make.shape[0]), x='cty', data=df_make, s=75, 
               edgecolors='gray', c='w', alpha=0.5)
    ax.scatter(y=i, x='cty', data=df_median.loc[df_median.index==make, :], s=75,
              c='firebrick')

#注释
ax.text(33, 13, "$red \; dots \; are \; the \: median$", fontsize=12, 
        color='firebrick')

#修饰图片
red_patch = plt.plot([], [], marker='o', ms=10, ls='', mec=None, color='firebrick',
                    label='Median')
plt.legend(handles=red_patch)
ax.set_title("Distribution of City Mileage by Make", fontsize=22)
ax.set_xlabel("Miles Per Gallon (city)", alpha=0.7)
ax.set_yticks(df.index)
ax.set_yticklabels(df.manufacturer.str.title(), alpha=0.7, 
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_80_0.png)
...
```python
"""绘制箱形图时需要注意解释可能会扭曲该组中包含的点数的框的大小，有必要手动提供
每个框中的观察数量，以克服这个缺点"""
#数据准备
df = pd.read_csv("mpg_ggplot2.csv")
#绘图
...
def add_n_obs(df, group_var, y):
    ...
    for (x, xticklabel),obs in zip(enumerate(xticklabels), n_obs):
        plt.text(x, medians_dict[xticklabel]*1.01, '#obs: '+str(obs), 
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_82_0.png)
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_84_0.png)
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_86_0.png)
...
colors = [plt.cm.Spectral(i/float(len(df[group_var].unique())-1)) for i in range(
        len(df[group_var].unique()))]
for c,group in zip(colors, df[group_var].unique()):
    sns.barplot(x='Users', y='Stage', data=df.loc[df[group_var]==group,:], 
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_88_0.png)
...
```python
"""由seaborn库提供的分类图，可用于可视化彼此相关的两个或多个分类变量的计数分布"""
#数据准备
titanic = sns.load_dataset('titanic')
#绘图
g = sns.catplot('alive', col='deck', col_wrap=4, kind='count', height=3.5, 
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_90_0.png)



```python
#绘图例2
sns.catplot(x='age', y='embark_town', hue='sex', col='class', orient='h', height=5,
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_95_0.png)
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_97_0.png)
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_100_0.png)
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_102_0.png)
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_105_0.png)
...
for i,val in enumerate(df["counts"].values):
    plt.text(i, val, float(val), horizontalalignment='center', verticalalignment=
            'bottom', fontdict={"fontweight":500, "size":12})
#修饰图片
plt.gca().set_xticklabels(df["manufacturer"], rotation=60, horizontalalignment=
                          'right')
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_108_0.png)
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_111_0.png)
...
plt.scatter(df.date[peak_locations], df.traffic[peak_locations], marker=
            mpl.markers.CARETUPBASE, color='tab:green', s=100, label='Peaks')
plt.scatter(df.date[trough_locations], df.traffic[trough_locations], marker=
            mpl.markers.CARETUPBASE, color='tab:red', s=100, label='Troughs')
#注释
for t,p in zip(trough_locations[1::5], peak_locations[::3]):
    plt.text(df.date[p], df.traffic[p]+15, df.date[p], horizontalalignment=
            'center', color='darkgreen')
    plt.text(df.date[t], df.traffic[t]-35, df.date[t], horizontalalignment=
            'center', color='darkred')
#修饰图片
plt.ylim(50, 750)
xtick_location = df.index.tolist()[::6]
xtick_labels = df.date.tolist()[::6]
plt.xticks(ticks=xtick_location, labels=xtick_labels, rotation=90, fontsize=12, 
          alpha=0.7)
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_114_0.png)
...
```python
"""
    自相关图（ACF）显示时间序列与其自身滞后的相关性，图中蓝色阴影区域是显著性水平
位于蓝线之上的滞后是显著的滞后。
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_117_0.png)
...
# ref: https://stats.stackexchange.com/questions/3115/cross-correlation-
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_123_0.png)
...
plt.title('Number of Deaths from Lung Disease in the UK (1974-1979)', fontsize=22)
plt.yticks(range(y_LL, y_UL, y_interval), [str(y) for y in 
            range(y_LL, y_UL, y_interval)], fontsize=22)
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_126_0.png)
...
ax2.set_title('Personal Savings Rate vs Unemployed(in Secondary Y Axis)', 
              fontsize=22)
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_129_0.png)
...
for y in range(8, 20, 2):
    plt.hlines(y, xmin=s, xmax=e, color='black', alpha=0.5, linestyle='--', lw=0.5)

plt.show()
```


![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_132_0.png)
...
from scipy.stats import sem
#数据准备
df_raw = pd.read_csv('orders_45d.csv', parse_dates=['purchase_time','purchase_date'])
...
plt.title('Daily Order Quantity of Brazilian Retail with Error Bands', fontsize=20)
...
for y in range(5, 10, 1):
    plt.hlines(y, xmin=s ,xmax=e, colors='black', alpha=0.5, linestyle='--', lw=0.5)

plt.show()
```


![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_134_0.png)
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_137_0.png)
...
#绘图
fig, ax = plt.subplots(1, 1, figsize=(16,9), dpi=80)
ax.fill_between(x, y1=y1, y2=0, label=columns[1], alpha=0.5, color=mycolors[1],
               linewidth=2)
ax.fill_between(x, y1=y2, y2=0, label=columns[0], alpha=0.5, color=mycolors[0],
               linewidth=2)
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_140_0.png)
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_146_0.png)
...
dend = sch.dendrogram(sch.linkage(df[['Murder','Assault','UrbanPop','Rape']],
                                 method='ward'), 
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_149_0.png)
...
```python
"""用于划分属于同一群集的点"""
from sklearn.cluster import AgglomerativeClustering#Agglomerative--集聚
from scipy.spatial import ConvexHull
#数据准备
df = pd.read_csv("USArrests.csv")
#划分聚类
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_153_0.png)
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_156_0.png)
...
```python
"""平行坐标有助于可视化特征是否有助于有效地隔离组，如果实现隔离，则该特征在预测该组
时非常有用"""
...
![png](/img/in-post/Matplotlib/matplotlib绘图例子/output_158_0.png)
